...
1. 隐式类型转换问题
2. or 问题
3. order by 导致索引的错误选择。使用业务中没有排序需求，但是使用 order by 导致索引选错  
- explain: 
    - Using temporary ,需要使用零时表存储结果集。常见于排序和分组查询 
...
总请求量：高峰时间段每分钟 18k  每秒三百请求，四台api 机器 三台定时任务 平时 7-8k
700-800w 的请求， 早上八点到11点有两百万左右
透传的请求量： 160w/天 高峰时期 瞬时5k  3k/ 分钟  平均 1.5- 1.8k/ 分钟

受制于资金方的系统性能：甘肃直连在一分钟内请求量达到 七百左右时就会导致请求超时 有的资金方在一分钟两千的请求量也不会超时
